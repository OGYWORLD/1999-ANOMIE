...
void EndingScene::CitizenEnding(InfoHandler* info)
{
	...
	system("cls");
	to->SetColor(14);
	to->GoToXYPosition(ENDING_TEXT_POSITION_X - NameSize/2, ENDING_TEXT_POSITION_Y);
	std::cout << "제 15대 대통령이었던 " << info->GetPresidentName() << "의 탄핵 결정이 오늘 오전 9시에 결정되었습니다.";
	Sleep(2500);

	to->GoToXYPosition(ENDING_TEXT_POSITION_X - 5, ENDING_TEXT_POSITION_Y + 3);
	...
	to->GoToXYPosition(ENDING_TEXT_POSITION_X + 8 - CountrySize/2, ENDING_TEXT_POSITION_Y + 9);
	...
	to->SetColor(95);
	to->GoToXYPosition(ENDING_TEXT_POSITION_X + 27, ENDING_TEXT_POSITION_Y + 7);
...
void EndingScene::ArmyEnding(InfoHandler* info)
{
	...
	system("cls");
	to->SetColor(14);
	to->GoToXYPosition(ENDING_TEXT_POSITION_X + 3, ENDING_TEXT_POSITION_Y);
	std::cout  << info->GetYear() <<"년 " << info->GetMonth() << "월 " << info->GetDay() << "일, 김찬령 준장을 비롯한 91사단이 주축이 되어";
	Sleep(2500);

	to->GoToXYPosition(ENDING_TEXT_POSITION_X - 9 - NameSize / 2, ENDING_TEXT_POSITION_Y + 3);
	std::cout << "정권 점령, 사실상 쿠데타를 일으켰습니다. " << info->GetPresidentName() << "대통령은 약 오전 10시경 대통령직을 사퇴했음을";
	Sleep(3000);

	to->GoToXYPosition(ENDING_TEXT_POSITION_X, ENDING_TEXT_POSITION_Y + 6);
...
void EndingScene::ReligionEnding(InfoHandler* info)
{
	...
	to->GoToXYPosition(ENDING_TEXT_POSITION_X - 11, ENDING_TEXT_POSITION_Y + 3);
	std::cout << "사실상, 정부의 권위는 잃은지 오래고 종교 단체에 의한 국가 운영이 이루어지고 있던 셈입니다.";
	Sleep(3000);

	to->GoToXYPosition(ENDING_TEXT_POSITION_X - 12, ENDING_TEXT_POSITION_Y + 6);
	std::cout << "이미 대부분의 국민들은 국가의 신뢰를 잃은지 오래되었고 정부의 기능은 종교가 담당하고 있었습니다.";
	Sleep(3000);

	to->GoToXYPosition(ENDING_TEXT_POSITION_X + 10 - CountrySize / 2, ENDING_TEXT_POSITION_Y + 9);
	...
	to->SetColor(176);
	to->GoToXYPosition(ENDING_TEXT_POSITION_X + 15, ENDING_TEXT_POSITION_Y + 7);
...
void EndingScene::AllDieEnding(InfoHandler* info)
{
	...
	system("cls");
	to->SetColor(14);
	to->GoToXYPosition(ENDING_TEXT_POSITION_X - CountrySize / 2, ENDING_TEXT_POSITION_Y);
	...
	to->GoToXYPosition(ENDING_TEXT_POSITION_X + 2 - CountrySize / 2, ENDING_TEXT_POSITION_Y+1);
	std::cout << "(" << info->GetYear() << "년 " << info->GetMonth()+1 << "월 " << info->GetDay() << "일. 유엔은 " << info->GetCountryName() << "의 소멸을 공식적으로 선언하였습니다.)";
	Sleep(2500);

	to->GoToXYPosition(ENDING_TEXT_POSITION_X - 12 - NameSize/2, ENDING_TEXT_POSITION_Y + 4);
	...
	to->GoToXYPosition(ENDING_TEXT_POSITION_X - 14 - NameSize / 2, ENDING_TEXT_POSITION_Y + 5);
	std::cout << "(" << info->GetCountryName() << "(은)는 많은 좀비(리벌 바이러스) 재해를 겪었고 대통령인 " << info->GetPresidentName() << "(을)를 포함해서 모든 국민이 사망했습니다.)";
	Sleep(3000);

	to->GoToXYPosition(ENDING_TEXT_POSITION_X - 13 - CountrySize / 2, ENDING_TEXT_POSITION_Y + 8);
	...
	to->GoToXYPosition(ENDING_TEXT_POSITION_X - 5 - CountrySize / 2, ENDING_TEXT_POSITION_Y + 9);
	std::cout << "(유엔군은 한 달동안 " << info->GetCountryName() << "에서 생존자를 찾았지만 어떠한 생존자도 찾을 수 없었습니다.)";
	Sleep(3000);

	to->GoToXYPosition(ENDING_TEXT_POSITION_X + 10 - NameSize / 2, ENDING_TEXT_POSITION_Y + 12);
	std::cout << "So, UN judge " << info->GetCountryName() << "was destory by rebirth virus.";
	to->GoToXYPosition(ENDING_TEXT_POSITION_X - 2 - NameSize / 2, ENDING_TEXT_POSITION_Y + 13);
	...
	to->SetColor(79);
	to->GoToXYPosition(ENDING_TEXT_POSITION_X + 28, ENDING_TEXT_POSITION_Y + 7);
	std::cout << "당신은 죽었습니다!";
	Sleep(1000);
...
void EndingScene::PrintResult(InfoHandler* info)
{
	...
	music->PlayMoveBeep();
	to->SetColor(14);
	to->GoToXYPosition(ENDING_TEXT_POSITION_X + 22 - (NameSize + CountrySize) / 2, ENDING_TEXT_POSITION_Y + 12);
	std::cout << info->GetCountryName() << "의 대통령 " << info->GetPresidentName() << "  (재임일수: " << info->GetCntArr()[ECOUNT::DAYCOUNT] << ")";
	Sleep(1000);

	music->PlayMoveBeep();
	to->SetColor(15);
	to->GoToXYPosition(ENDING_TEXT_POSITION_X - 32, ENDING_TEXT_POSITION_Y + 15);
	...
	music->PlayMoveBeep();
	to->GoToXYPosition(ENDING_TEXT_POSITION_X - 25, ENDING_TEXT_POSITION_Y + 21);
	...
	music->PlayMoveBeep();
	to->GoToXYPosition(ENDING_TEXT_POSITION_X + 4, ENDING_TEXT_POSITION_Y + 24);
